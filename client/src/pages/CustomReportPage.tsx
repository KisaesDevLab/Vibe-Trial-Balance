...
function ReportViewer({ config, periodId }: { config: ReportConfig; periodId: number }) {
  ...
  const thCls = 'px-3 py-2 text-right text-xs font-semibold text-gray-500 uppercase tracking-wide border-b border-gray-200';
  const tdNum = 'px-3 py-1.5 text-sm text-right tabular-nums';
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-3 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide border-b border-gray-200">
                Account
              </th>
          ...
          <tbody>
            {config.sections.map(section => {
              ...
              return [
                <tr key={`${section.id}-hdr`} className="bg-teal-50">
                  <td colSpan={numCols + 1}
                    className="px-3 py-1.5 text-xs font-bold text-teal-800 uppercase tracking-wide">
                    {section.name}
                  </td>
                </tr>,
                ...
                section.showSubtotal && (
                  <tr key={`${section.id}-sub`} className="border-t-2 border-gray-200 bg-gray-50 font-semibold">
                    <td className="px-3 py-1.5 text-sm">Total {section.name}</td>
                    {subtotals.map((t, i) => (
...
function Builder({
  ...
  const inputCls = 'border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-teal-500';
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-5 gap-4 min-h-96">
        ...
        <div className="col-span-2 border border-gray-200 rounded-lg overflow-hidden flex flex-col">
          <div className="p-2 border-b border-gray-100 space-y-2 bg-gray-50">
          ...
          <div className="flex-1 overflow-y-auto">
            {filtered.map(a => (
              <div key={a.id}
                ...
                className={`px-3 py-2 text-xs border-b border-gray-100 flex items-center gap-2 ${
                  usedIds.has(a.id)
                    ? 'text-gray-300 cursor-not-allowed'
                    : 'hover:bg-teal-50 cursor-pointer text-gray-800'
                }`}>
                <span className="text-gray-400 w-16 shrink-0">{a.account_number}</span>
        ...
        <div className="col-span-3 space-y-3">
          ...
          {sections.map(sec => (
            <div key={sec.id}
              className={`border rounded-lg overflow-hidden ${activeSection === sec.id ? 'border-teal-400' : 'border-gray-200'}`}
              ...
              <div className="flex items-center gap-2 px-3 py-2 bg-gray-50 border-b border-gray-200">
                <input
                  ...
                  className="flex-1 text-sm font-medium bg-transparent border-0 focus:outline-none focus:bg-white focus:border focus:border-teal-400 focus:rounded px-1"
                />
              ...
              <div className="divide-y divide-gray-100">
                ...
                {sec.accountIds.map(id => {
                  ...
                  if (!a) return null;
                  return (
                    <div key={id} className="flex items-center px-3 py-1.5 text-xs hover:bg-gray-50">
                      <span className="text-gray-400 mr-2 w-14 shrink-0">{a.account_number}</span>
...
export function CustomReportPage() {
  ...
  if (!selectedClientId) {
    return <div className="p-8 text-center text-gray-400 text-sm">Select a client to manage custom reports.</div>;
  }

  return (
    <div className="p-6 space-y-4 max-w-6xl">
      <h1 className="text-lg font-bold text-gray-800">Custom Reports</h1>
      ...
      {tab === 'saved' && (
        <div className="space-y-3">
          ...
          {reports.map(report => (
            <div key={report.id} className="border border-gray-200 rounded-lg p-4 flex items-start justify-between">
              <div>
          ...
          {previewReport && selectedPeriodId && (
            <div className="mt-4">
              <h2 className="text-sm font-semibold text-gray-700 mb-3">{previewReport.name}</h2>
