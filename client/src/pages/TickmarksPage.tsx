...
function TickmarkForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="flex items-end gap-2 flex-wrap">
      ...
      <div>
        <label className="block text-xs text-gray-500 mb-0.5">Order</label>
        <input
          type="number"
          value={sortOrder}
          onChange={(e) => setSortOrder(e.target.value)}
          className="border border-gray-300 rounded px-2 py-1.5 text-sm w-16 focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
      </div>
...
export function TickmarksPage() {
  ...
  return (
    <div className="p-6 max-w-3xl">
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-gray-900">Tickmark Library</h2>
      ...
      <div className="bg-white rounded-lg border border-gray-200 px-5 py-4 mb-6">
        <h3 className="text-sm font-semibold text-gray-700 mb-3">Add Tickmark</h3>
        <TickmarkForm
          onSave={(input) => createMut.mutate(input)}
          saving={createMut.isPending}
        />
        {createMut.isError && (
          <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded mt-2 text-sm">{(createMut.error as Error).message}</div>
        )}
      </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-lg border border-gray-200 px-5 py-10 text-center text-gray-400 text-sm">
        ...
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-500 w-20">Symbol</th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-500">Description</th>
                <th className="px-4 py-2.5 text-center text-xs font-semibold text-gray-500 w-16">Order</th>
                <th className="px-4 py-2.5 text-right text-xs font-semibold text-gray-500 w-28">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {tickmarks.map((tm) => (
                editId === tm.id ? (
                  <tr key={tm.id} className="bg-blue-50/40">
                  ...
                  <tr key={tm.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2.5 text-gray-700">{tm.description}</td>
                    <td className="px-4 py-2.5 text-center text-gray-500">{tm.sort_order}</td>
