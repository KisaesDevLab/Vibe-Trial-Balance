...
const BADGE_ACTIVITY: Record<string, string> = {
  ...
  farm_rental: 'bg-teal-50 text-teal-700',
  common: 'bg-gray-100 text-gray-600',
};
...
function YesNoBadge({ value }: { value: boolean }) {
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${value ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
      {value ? 'Yes' : 'No'}
    </span>
  );
}
...
function SoftwareMappingsTable({
  ...
  return (
    <div>
      <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Software Mappings</p>
      <div className="border border-gray-200 rounded overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-3 py-2 text-left text-xs font-semibold text-gray-600 w-28">Software</th>
              <th className="px-3 py-2 text-left text-xs font-semibold text-gray-600">Software Code</th>
              <th className="px-3 py-2 text-left text-xs font-semibold text-gray-600">Software Description</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {rows.map((row, idx) => (
              <tr key={row.software}>
                <td className="px-3 py-1.5 text-xs text-gray-700 font-medium">{SOFTWARE_LABELS[row.software]}</td>
      ...
      <p className="text-xs text-gray-400 mt-1">Changes auto-save on blur. Clearing both fields removes the mapping.</p>
    </div>
  );
}
...
function EditPanel({ initial, onSave, onCancel, onDelete, saving, error, mapRefresh }: EditPanelProps) {
  ...
  return (
    <div className="border border-gray-200 rounded-lg bg-gray-50 p-4 mt-4">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="flex items-center justify-between mb-1">
          <h3 className="text-sm font-semibold text-gray-800">
            {initial ? 'Edit Tax Code' : 'New Tax Code'}
            {isSystem && (
              <span className="ml-2 text-xs font-normal text-amber-600 bg-amber-50 px-2 py-0.5 rounded">System</span>
            )}
          </h3>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Return Form</label>
            <select
          ...
          <div>
            ...
            <input
              ...
              readOnly={isSystem}
              className={`w-full border border-gray-300 rounded px-2 py-1.5 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500 ${isSystem ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : ''}`}
              required
            />
          </div>
          <div>
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          <div className="flex gap-2">
            <button type="button" onClick={onCancel} className="px-3 py-1.5 text-sm border border-gray-300 rounded hover:bg-gray-100">
              Cancel
            </button>
            <button
              type="submit"
        ...
        {result?.data ? (
          ...
          <>
            <p className="text-sm text-gray-600">
              CSV must have columns: <code className="bg-gray-100 px-1 rounded text-xs">tax_code, return_form, activity_type, description</code>. Optional: <code className="bg-gray-100 px-1 rounded text-xs">sort_order, notes</code>.
            </p>
            ...
            >
              ...
              {fileName ? (
                ...
                <p className="text-sm text-gray-400">Click or drag a CSV file here</p>
              )}
            </div>
            ...
            {preview && (
              <>
                <div className="flex items-center gap-4 text-sm">
                  ...
                  <span className="text-yellow-700 font-medium">{updateCount} updates</span>
                  ...
                  <table className="w-full text-xs">
                    <thead className="bg-gray-50 sticky top-0">
                      <tr>
                        ...
                        <th className="px-2 py-1.5 text-left font-semibold text-gray-500">Tax Code</th>
                        <th className="px-2 py-1.5 text-left font-semibold text-gray-500">Form</th>
                        <th className="px-2 py-1.5 text-left font-semibold text-gray-500">Activity</th>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {preview.slice(0, 100).map((row, i) => (
                        <tr key={i} className={PREVIEW_STATUS_CLASSES[row.status]}>
                          <td className="px-2 py-1 font-medium text-gray-600">{PREVIEW_STATUS_LABELS[row.status]}</td>
...
export function TaxCodesPage() {
  ...
  const codes = listData ?? [];

  const totalCodes = codes.length;
  const activeCodes = codes.filter((c) => c.is_active).length;
  const systemCodes = codes.filter((c) => c.is_system).length;
  const customCodes = codes.filter((c) => !c.is_system).length;
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-start justify-between mb-4">
        <div>
          <h2 className="text-xl font-semibold text-gray-900">Tax Code Management</h2>
          <p className="text-sm text-gray-500 mt-0.5">System-wide tax code library and software mappings</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="px-3 py-1.5 text-sm border border-gray-300 rounded hover:bg-gray-50 inline-block"
          >
            Export CSV
          </a>
          <button
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        <input
          ...
          className="border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 min-w-52"
        />
      ...
      <div className="grid grid-cols-4 gap-3 mb-4">
        {[
          ...
          { label: 'Custom', value: customCodes },
        ].map((s) => (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  <th className="px-3 py-2.5 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Return Form</th>
                  <th className="px-3 py-2.5 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Activity</th>
                  <th className="px-3 py-2.5 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Tax Code</th>
                  <th className="px-3 py-2.5 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Description</th>
                  <th className="px-3 py-2.5 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider w-16">Sort</th>
                  <th className="px-3 py-2.5 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider w-16">System</th>
                  <th className="px-3 py-2.5 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider w-16">Active</th>
                  <th className="px-3 py-2.5 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider w-24">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {codes.length === 0 ? (
                  ...
                  codes.map((code) => (
                    <tr
                      key={code.id}
                      className={`hover:bg-gray-50 transition-colors cursor-pointer ${editId === code.id ? 'bg-blue-50' : ''}`}
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5">
                        <span className="font-mono font-bold text-gray-900">{code.tax_code}</span>
                      </td>
                      <td className="px-3 py-2.5 text-gray-700 max-w-xs truncate">{code.description}</td>
                      <td className="px-3 py-2.5 text-gray-500">{code.sort_order}</td>
