...
export function TaxCodeReportPage() {
  ...
  const handleExport = () => {
    ...
    downloadCsv(`tax-code-report-${selectedPeriodId}.csv`, [header, ...exportRows]);
  };
  ...
  return (
    <div className="p-6 max-w-3xl">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-semibold text-gray-900">Tax Code Report</h2>
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {sortedGroups.map(([code, codeRows]) => {
            const subtotal = codeRows.reduce((s, r) => s + netBalance(r, colSet), 0);
            const sorted = [...codeRows].sort((a, b) => a.sort_order - b.sort_order || a.account_number.localeCompare(b.account_number));
            return (
              <div key={code} className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                <div className={`px-4 py-2 border-b flex items-center justify-between ${code === 'Unassigned' ? 'bg-gray-100 border-gray-300' : 'bg-blue-50 border-blue-200'}`}>
                  <span className={`text-sm font-bold ${code === 'Unassigned' ? 'text-gray-500 italic' : 'text-blue-900'}`}>{code}</span>
                  <span className="text-sm font-mono font-semibold text-gray-800">{fmt(subtotal)}</span>
                </div>
                <table className="w-full text-sm">
                  <tbody className="divide-y divide-gray-100">
                    {sorted.map((r) => (
                      <tr key={r.account_id} className="hover:bg-gray-50">
                        <td className="px-4 py-1.5 text-sm text-gray-600 w-28 whitespace-nowrap">{r.account_number}</td>
                        <td className="px-3 py-1.5 text-sm text-gray-700 flex-1">{r.account_name}</td>
                        <td className="px-3 py-1.5 text-xs text-gray-400 w-24 capitalize">{r.category}</td>
                        <td className="px-4 py-1.5 text-sm font-mono text-right text-gray-700 w-36">{fmt(netBalance(r, colSet))}</td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot>
                    <tr className="border-t border-gray-300 bg-gray-50">
                      <td colSpan={3} className="px-4 py-1.5 text-xs font-semibold text-gray-600">Subtotal — {code}</td>
                      <td className="px-4 py-1.5 text-sm font-mono font-semibold text-right text-gray-800 border-t border-gray-400">{fmt(subtotal)}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
          ...
          {/* Grand total */}
          <div className="bg-gray-100 rounded-lg border-2 border-gray-700 px-4 py-3 flex items-center justify-between">
            <span className="text-sm font-bold text-gray-900">Grand Total</span>
            <span className="text-sm font-mono font-bold text-gray-900">{fmt(grandTotal)}</span>
          </div>
        </div>
      )}
    </div>
