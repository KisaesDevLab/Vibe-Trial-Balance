...
const TYPE_BADGE: Record<string, { label: string; cls: string }> = {
  book:  { label: 'Book AJE',    cls: 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-400' },
  tax:   { label: 'Tax AJE',     cls: 'bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-400' },
  trans: { label: 'Transaction', cls: 'bg-teal-100 dark:bg-teal-900/40 text-teal-700 dark:text-teal-400' },
};
        ...
        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
          {entry.lines.map((line, i) => (
            <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
              <td className="px-4 py-1.5 text-sm font-mono text-gray-600 dark:text-gray-400 w-28 whitespace-nowrap">{line.account_number}</td>
              <td className="px-3 py-1.5 text-sm text-gray-700 dark:text-gray-300">{line.account_name}</td>
              <td className="px-4 py-1.5 text-right text-sm font-mono text-gray-700 dark:text-gray-300 w-36">{line.debit > 0 ? fmt(line.debit) : ''}</td>
              <td className="px-4 py-1.5 text-right text-sm font-mono text-gray-700 dark:text-gray-300 w-36 pl-8">{line.credit > 0 ? fmt(line.credit) : ''}</td>
            </tr>
          ))}
        </tbody>
        <tfoot>
    ...
    <div className="p-6 max-w-3xl">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleExport} disabled={!entries.length} className="px-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded hover:bg-gray-50 dark:hover:bg-gray-700/50 dark:text-gray-300 disabled:opacity-40">Export Excel</button>
          <button
      ...
      {isLoading ? (
        ...
        <>
          ...
          {/* Grand total footer */}
          <div className="bg-gray-100 dark:bg-gray-700 rounded-lg border-2 border-gray-700 dark:border-gray-500 overflow-hidden">
            <table className="w-full text-sm">
              <tbody>
                <tr>
                  <td className="px-4 py-2.5 text-sm font-bold text-gray-900 dark:text-white">Total — {entries.length} entr{entries.length === 1 ? 'y' : 'ies'}</td>
                  <td className="px-4 py-2.5 text-right text-sm font-mono font-bold text-gray-900 dark:text-white w-36">{fmt(totalDebit)}</td>
                  <td className="px-4 py-2.5 text-right text-sm font-mono font-bold text-gray-900 dark:text-white w-36 pl-8">{fmt(totalCredit)}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          {(() => {
            ...
            return accountSummary.length > 0 && (
              <div className="mt-6">
                <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Summary by Account</h3>
                <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 dark:bg-gray-800/60 border-b border-gray-200 dark:border-gray-700">
                      <tr>
                        <th className="px-4 py-2 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 w-28">Acct #</th>
                        <th className="px-3 py-2 text-left text-xs font-semibold text-gray-500 dark:text-gray-400">Account Name</th>
                        <th className="px-4 py-2 text-right text-xs font-semibold text-gray-500 dark:text-gray-400 w-36">Total Dr</th>
                        <th className="px-4 py-2 text-right text-xs font-semibold text-gray-500 dark:text-gray-400 w-36">Total Cr</th>
                        <th className="px-4 py-2 text-right text-xs font-semibold text-gray-500 dark:text-gray-400 w-36">Net</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                      {accountSummary.map(({ account, name, debit, credit }) => {
                        ...
                        return (
                          <tr key={account} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            <td className="px-4 py-1.5 text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">{account}</td>
                            <td className="px-3 py-1.5 text-sm text-gray-700 dark:text-gray-300">{name}</td>
                            <td className="px-4 py-1.5 text-right text-sm font-mono text-gray-700 dark:text-gray-300">{debit > 0 ? fmt(debit) : '—'}</td>
                            <td className="px-4 py-1.5 text-right text-sm font-mono text-gray-700 dark:text-gray-300">{credit > 0 ? fmt(credit) : '—'}</td>
                            <td className={`px-4 py-1.5 text-right text-sm font-mono font-semibold ${net > 0 ? 'text-gray-800 dark:text-gray-200' : net < 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-400 dark:text-gray-500'}`}>
