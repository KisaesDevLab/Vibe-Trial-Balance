...
function parseCents(v: string): number {
  const evaled = evalAmountExpr(v);
  const n = parseFloat(evaled.replace(/[$,]/g, ''));
...
function M1Modal({ initial, onClose, onSave }: ModalProps) {
  ...
  const inputErrCls = 'w-full border border-red-400 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-red-500 dark:bg-gray-700 dark:text-white';
  const labelCls = 'block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1';

  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md p-6 space-y-4">
        <h2 className="text-base font-semibold dark:text-white">{initial ? 'Edit Adjustment' : 'Add M-1 Adjustment'}</h2>
        ...
        {amountError && (
          <p className="text-xs text-red-600 dark:text-red-400 -mt-2">{amountError}</p>
        )}
...
function M1WorksheetTab({ periodId }: { periodId: number }) {
  ...
  const numCls = `${tdCls} text-right font-mono tabular-nums`;
  const summaryRow = 'flex justify-between items-center py-1.5 text-sm border-b border-gray-100 dark:border-gray-700';

  return (
    <div className="space-y-4">
      {/* Toolbar */}
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300">M-1 Book-to-Tax Adjustments</h2>
          ...
          <tbody>
              ...
              return (
                <tr key={a.id} className="border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  <td className={tdCls}>
                    ...
                    {a.notes && <div className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{a.notes}</div>}
                  </td>
                  <td className={tdCls}>
                    {a.category && (
                      <span className="inline-block px-2 py-0.5 text-xs bg-gray-100 dark:bg-gray-700 dark:text-gray-300 rounded-full">
                        {a.category}
                      </span>
                    )}
                  </td>
                  ...
                  <td className={`${numCls} ${diff > 0 ? 'text-red-600 dark:text-red-400' : diff < 0 ? 'text-green-700 dark:text-green-400' : ''}`}>
                    {fmtDiff(diff)}
                  </td>
            ...
            {adjs.length > 0 && (
              <tr className="border-t-2 border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800/60 font-semibold">
                ...
                <td className={`${numCls} ${totalDiff > 0 ? 'text-red-600 dark:text-red-400' : totalDiff < 0 ? 'text-green-700 dark:text-green-400' : ''}`}>
                  {fmtDiff(totalDiff)}
                </td>
                <td />
              </tr>
      ...
      {/* Summary */}
      <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-800/60 max-w-sm">
        <h3 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">
          Reconciliation Summary
        </h3>
        <div className={summaryRow}>
          <span className="text-gray-600 dark:text-gray-400">Book Net Income</span>
          <span className="text-sm font-mono font-medium tabular-nums dark:text-gray-300">{fmt(bookNetIncome)}</span>
        </div>
        <div className={summaryRow}>
          <span className="text-gray-600 dark:text-gray-400">Net M-1 Adjustments</span>
          <span className={`text-sm font-mono font-medium tabular-nums ${totalDiff > 0 ? 'text-red-600 dark:text-red-400' : totalDiff < 0 ? 'text-green-700 dark:text-green-400' : 'dark:text-gray-300'}`}>
            {totalDiff >= 0 ? '+' : ''}{fmtDiff(totalDiff)}
          </span>
        </div>
        <div className="flex justify-between items-center pt-2 text-sm font-bold border-t border-gray-300 dark:border-gray-600 mt-1 dark:text-white">
          <span>Taxable Income</span>
          <span className="text-sm font-mono tabular-nums">{fmt(taxableIncome)}</span>
        </div>
        {Math.abs(taxableIncome - taxNetIncome) > 1 && (
          <div className="mt-2 text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-700 rounded px-2 py-1.5">
...
function TaxBasisTab({ periodId }: { periodId: number }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Tax Basis Schedule — Book vs. Tax Balances</h2>
        <div className="flex gap-2">
          <button onClick={exportExcel}
            className="px-3 py-1.5 text-xs border border-gray-300 dark:border-gray-600 rounded hover:bg-gray-50 dark:hover:bg-gray-700/50 dark:text-gray-300 font-medium">
            Export Excel
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {CATEGORY_ORDER.map(cat => {
              ...
              return [
                // Category header
                <tr key={`${cat}-hdr`} className="bg-teal-50 dark:bg-teal-900/20">
                  <td colSpan={4} className="px-3 py-1.5 text-xs font-bold text-teal-800 dark:text-teal-400 uppercase tracking-wide">
                    {CATEGORY_LABELS[cat]}
                  </td>
                </tr>,
                ...
                ...catRows.map(r => {
                  ...
                  return (
                    <tr key={r.account_id} className="border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      <td className={tdCls}>
                        <span className="font-mono text-gray-500 dark:text-gray-400 mr-2">{r.account_number}</span>
                        {r.account_name}
                      </td>
                      <td className={numCls}>{fmt(book)}</td>
                      <td className={numCls}>{fmt(tax)}</td>
                      <td className={`${numCls} ${Math.abs(diff) > 0 ? 'font-medium text-amber-700 dark:text-amber-400' : 'text-gray-400 dark:text-gray-500'}`}>
                        {diff !== 0 ? fmtDiff(diff) : '—'}
                      </td>
                    </tr>
                  );
                ...
                // Subtotal
                <tr key={`${cat}-sub`} className="border-t-2 border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-800/60 font-semibold">
                  <td className={tdCls}>Total {CATEGORY_LABELS[cat]}</td>
                  <td className={numCls}>{fmt(bookTotal)}</td>
                  <td className={numCls}>{fmt(taxTotal)}</td>
                  <td className={`${numCls} ${Math.abs(diffTotal) > 0 ? 'text-amber-700 dark:text-amber-400' : 'text-gray-400 dark:text-gray-500'}`}>
