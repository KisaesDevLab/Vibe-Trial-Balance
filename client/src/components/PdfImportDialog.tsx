...
import {
  ...
  type PdfAccountNumberSuggestion,
  type PdfMatchRow,
...
export function PdfImportDialog({ periodId, clientId, onClose, onSuccess }: Props) {
  ...
  // Ask for numbers a chunk of rows at a time — a whole statement in one
  // request stays open long enough for the proxy in front of the AI router to
  // abandon it with a 524. Suggestions come back matched by account name, so
  // chunk boundaries don't affect how they're applied.
  const SUGGEST_CHUNK_SIZE = 25;

  const needsNumber = (m: EditableMatch): boolean =>
    m.action === 'create_new' && !m.pdfAccountNumber?.trim() && !m.newAccountNumber?.trim();
  ...
  const handleAiAssignNumbers = async () => {
    ...
    try {
      ...
      const suggestions: PdfAccountNumberSuggestion[] = [];
      const reserved: string[] = [];
      let chunkError: string | null = null;

      for (let i = 0; i < pending.length; i += SUGGEST_CHUNK_SIZE) {
        setSuggestProgress({ done: i, total: pending.length });
        const result = await suggestPdfAccountNumbers(clientId, pending.slice(i, i + SUGGEST_CHUNK_SIZE), reserved);
      ...
      // Keep whatever landed before the failure — re-running only has to cover
