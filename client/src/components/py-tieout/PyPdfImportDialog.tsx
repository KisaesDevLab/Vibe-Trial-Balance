...
export function PyPdfImportDialog({ periodId, clientId, onClose, onSuccess }: Props) {
  const qc = useQueryClient();
  const [stage, setStage] = useState<Stage>('consent');
  const [error, setError] = useState<string | null>(null);
  const [matches, setMatches] = useState<PdfMatchRow[]>([]);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setError(null);
    try {
      const res = await analyzePdf(file, periodId, clientId);
  ...
  const handleConfirm = async () => {
    ...
    setError(null);
    try {
      const res = await confirmPdfPyImport(periodId, clientId, matches, sourceFilename);
  ...
  const updateMatch = (idx: number, updates: Partial<PdfMatchRow>) => {
  ...
  if (stage === 'consent') {
    return (
      <AiConsentDialog
        feature="PY Comparison PDF Import"
      ...
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-sm">
          <div className="px-5 py-4 border-b dark:border-gray-700">
            <h2 className="text-base font-semibold dark:text-white">PDF Import Complete</h2>
          </div>
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-visible">
        <div className="flex items-center justify-between px-5 py-4 border-b dark:border-gray-700 shrink-0">
          <div>
            <h2 className="text-base font-semibold dark:text-white">Import PY from PDF</h2>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
              {stage === 'upload' && 'Upload a PDF trial balance'}
              {stage === 'analyzing' && 'AI is extracting data from PDF...'}
              {stage === 'preview' && `${matches.length} rows extracted — review matches`}
          ...
          {stage === 'upload' && (
            <div className="text-center py-12">
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                Upload a PDF containing the prior year final trial balance.
              </p>
              <label className="inline-block">
                ...
                <input
                  ...
                  className="block text-sm text-gray-500 dark:text-gray-400 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border file:border-gray-300 dark:file:border-gray-600 file:text-sm file:font-medium file:bg-amber-50 dark:file:bg-gray-700 dark:file:text-gray-300 hover:file:bg-amber-100 dark:hover:file:bg-gray-600"
                />
              </label>
            </div>
          )}
          ...
          {stage === 'analyzing' && (
            <div className="flex items-center justify-center py-12">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">AI is extracting trial balance data from the PDF...</p>
              </div>
            </div>
          )}
                ...
                <table className="w-full text-xs">
                  <thead className="sticky top-0 bg-gray-50 dark:bg-gray-800/60">
                    <tr className="border-b border-gray-200 dark:border-gray-700">
                      <th className="px-2 py-1.5 text-left font-semibold text-gray-600 dark:text-gray-400">PDF Acct #</th>
                      <th className="px-2 py-1.5 text-left font-semibold text-gray-600 dark:text-gray-400">PDF Name</th>
                    ...
                    {matches.map((m, idx) => {
                      ...
                      return (
                        <tr key={idx} className={isUnmatched ? 'bg-amber-50/50 dark:bg-amber-900/10' : m.action === 'create_new' ? 'bg-blue-50/50 dark:bg-blue-900/10' : ''}>
                          <td className="px-2 py-1 font-mono text-gray-900 dark:text-white">{m.pdfAccountNumber ?? '—'}</td>
                          <td className="px-2 py-1 text-gray-700 dark:text-gray-300 truncate max-w-[10rem]">{m.pdfAccountName ?? '—'}</td>
                          <td className="px-2 py-1">
                            {m.action === 'skip' ? (
                              <span className="text-gray-400 dark:text-gray-500 text-xs italic">Skipped</span>
                            ) : m.action === 'create_new' ? (
                              <span className="text-blue-600 dark:text-blue-400 text-xs font-medium">
                                + New: {m.pdfAccountNumber ?? ''} — {m.pdfAccountName ?? ''}
                              </span>
                            ) : (
                              <AccountSearchDropdown
                                ...
                                onChange={(accountId) => {
                                  if (accountId === '') {
                                    updateMatch(idx, { matchedAccountId: null, action: 'skip' } as Partial<PdfMatchRow>);
                                  } else {
                          ...
                          <td className="px-2 py-1 text-center">
                            <input
                              ...
                              onChange={(e) => {
                                if (e.target.checked) {
                                  updateMatch(idx, { action: 'skip' } as Partial<PdfMatchRow>);
                                } else {
                                  updateMatch(idx, { action: 'match' } as Partial<PdfMatchRow>);
                                }
                              }}
