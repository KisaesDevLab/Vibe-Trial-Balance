...
// Serialise as a JS migration
const js = `exports.up = async function(knex) {
  // Remove the original 7 system templates (accounts cascade via FK)
  await knex('coa_templates').where({ is_system: true }).delete();
...
const outPath = path.join(__dirname, '../server/migrations/20260321000005_replace_system_coa_templates.js');
fs.writeFileSync(outPath, js, 'utf8');
console.log('Written:', outPath);
console.log('Templates:', templates.length, '  Total accounts:', templates.reduce((s, t) => s + t.accounts.length, 0));
