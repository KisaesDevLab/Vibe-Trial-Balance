...
exports.glPeriodRouter = void 0;
const express_1 = require("express");
const db_1 = require("../db");
const auth_1 = require("../middleware/auth");
exports.glPeriodRouter = (0, express_1.Router)({ mergeParams: true });
exports.glPeriodRouter.use(auth_1.authMiddleware);
...
exports.glPeriodRouter.get('/', async (req, res) => {
    ...
    try {
        ...
        const tbRows = await (0, db_1.db)('trial_balance as tb')
            .join('chart_of_accounts as coa', 'coa.id', 'tb.account_id')
            .where('tb.period_id', periodId)
            .select('coa.id as account_id', 'coa.account_number', 'coa.account_name', 'coa.category', 'coa.normal_balance', 'tb.unadjusted_debit', 'tb.unadjusted_credit')
            .orderBy('coa.account_number');
        // All JE lines for the period
        const jeLines = await (0, db_1.db)('journal_entry_lines as jel')
            .join('journal_entries as je', 'je.id', 'jel.journal_entry_id')
            .join('chart_of_accounts as coa', 'coa.id', 'jel.account_id')
            .where('je.period_id', periodId)
            .select('coa.id as account_id', 'coa.account_number', 'coa.account_name', 'coa.category', 'coa.normal_balance', 'je.entry_date', 'je.entry_number', 'je.entry_type', 'je.description', 'jel.debit', 'jel.credit')
            .orderBy(['coa.account_number', 'je.entry_date', 'je.entry_number']);
        const tbMap = new Map();
        for (const row of tbRows)
            tbMap.set(row.account_id, row);
        // Collect all unique accounts (TB + JE), preserving sort
        const accountOrder = new Map();
        for (const row of tbRows) {
            if (!accountOrder.has(row.account_id))
                accountOrder.set(row.account_id, row);
        }
        for (const line of jeLines) {
            if (!accountOrder.has(line.account_id))
                accountOrder.set(line.account_id, line);
        }
        ...
        const accounts = [...accountOrder.values()].sort((a, b) => a.account_number.localeCompare(b.account_number));
