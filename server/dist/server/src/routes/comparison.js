...
exports.comparisonRouter = (0, express_1.Router)({ mergeParams: true });
exports.comparisonRouter.use(auth_1.authMiddleware);
function netBal(dr, cr, normalBalance) {
    return normalBalance === 'debit' ? dr - cr : cr - dr;
}
// GET /api/v1/periods/:periodId/compare/:comparePeriodId
exports.comparisonRouter.get('/', async (req, res) => {
    ...
    try {
        const [period, comparePeriod] = await Promise.all([
            (0, db_1.db)('periods').where({ id: periodId }).first('id', 'period_name', 'start_date', 'end_date', 'client_id'),
            (0, db_1.db)('periods').where({ id: comparePeriodId }).first('id', 'period_name', 'start_date', 'end_date', 'client_id'),
        ]);
        ...
        const [currentRows, compareRows] = await Promise.all([
            (0, db_1.db)('v_adjusted_trial_balance as vtb')
        ...
        const notes = await (0, db_1.db)('variance_notes')
        ...
        const rows = currentRows.map((r) => {
            ...
            const variancePct = compareBalance !== 0
...
exports.comparisonRouter.put('/variance-notes/:accountId', async (req, res) => {
    const periodId = Number(req.params.periodId);
    const comparePeriodId = Number(req.params.comparePeriodId);
