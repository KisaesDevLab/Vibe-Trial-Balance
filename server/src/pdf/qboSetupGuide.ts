...
export function buildQboSetupGuideContent(input: SetupGuideInput): Content[] {
  ...
  return [
    ...
    kv([
      ...
      ['Credentials', input.configured ? (input.envOverride ? 'Set via environment variables' : 'Set on the QuickBooks page') : 'Not configured yet'],
    ]),
    ...
    bullets([
      ...
      'The Environment setting on the QuickBooks page must match the key set entered: sandbox keys with Sandbox, production keys with Production.',
    ...
    steps([
      'Open Setup → QuickBooks as an administrator.',
    ...
    bullets([
      'In the Intuit app, complete the App assessment questionnaire on the Production tab. This installation is a private, unlisted app used by one firm with a read-only accounting scope; it does not need to be published to the QuickBooks App Store.',
      'Intuit requires a public HTTPS redirect URI, a privacy policy URL and an end-user licence URL for production keys. A short page on the firm\'s website is sufficient for both.',
      'Set APP_BASE_URL on the server to the public HTTPS address (or set the redirect URI override on the QuickBooks page), register that redirect URI under Production, switch the Environment to Production and enter the production keys.',
    ]),

    h('6. Connecting a client company'),
    steps([
      'On the QuickBooks page, find the client in the Connections table and press Connect.',
    ...
    h('8. Troubleshooting'),
    kv([
      ['redirect_uri mismatch / "Something went wrong" at Intuit', 'The effective redirect URI shown on the QuickBooks page is not registered on the Intuit app for the selected environment. Copy it exactly, including https and the full path.'],
