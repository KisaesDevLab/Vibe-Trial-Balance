...
export function buildQboSetupGuideContent(input: SetupGuideInput): Content[] {
  ...
  return [
    ...
    bullets([
      ...
      'Accounts are matched by their stored QuickBooks account id, then by the QuickBooks account number against the chart of accounts, then by an identical name (flagged "by name" for review). A company that never turned on account numbers in QuickBooks matches by name only. Anything else is offered as a new account typed from QuickBooks\' classification, can be pointed at an existing account by hand, or — when AI is configured — can be suggested by "Suggest matches with AI", which sees account names and types only and whose suggestions the reviewer confirms one by one.',
      'Rows with a zero balance are skipped by default; untick "Skip zero-balance accounts" in the preview to include them.',
