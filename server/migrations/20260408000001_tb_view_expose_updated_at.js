/**
 * Migration: Expose trial_balance.updated_at on v_adjusted_trial_balance.
 *
 * Needed for optimistic-concurrency checks on TB cell edits: the client sends
 * back the updated_at it saw when loading the row, and the server rejects a
 * write if the row has changed since. Without this column in the view, the
 * client has no snapshot to send back.
 */
