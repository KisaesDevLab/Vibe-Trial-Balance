/**
 * Migration: Add prior_year_debit/credit to trial_balance; expose in view
 */
exports.up = async function (knex) {
  await knex.schema.table('trial_balance', (t) => {
    t.bigInteger('prior_year_debit').defaultTo(0).notNullable();
    t.bigInteger('prior_year_credit').defaultTo(0).notNullable();
  });
...
exports.down = async function (knex) {
  await knex.raw('DROP VIEW IF EXISTS v_adjusted_trial_balance');
  // Restore previous view (without prior_year columns)
  await knex.raw(`CREATE VIEW v_adjusted_trial_balance AS
    SELECT
      tb.period_id,
      tb.account_id,
  ...
  await knex.schema.table('trial_balance', (t) => {
    t.dropColumn('prior_year_debit');
    t.dropColumn('prior_year_credit');
  });
};
